...
**26.1.2** is the primary target for current Lunar Fabric profiles. **1.8.9 Forge** targets Lunar Client's 1.8.9 profile (press **`O`** on the Lunar home screen). See [docs/LUNAR.md](docs/LUNAR.md) for Lunar-specific setup.
...
1. In the Lunar launcher, select **1.8.9** and enable the **Forge** module.
2. Open version **Settings** (gear) → **Mods**, click the **folder** button, and copy in `forge-1.8\build\libs\IAS-9.0.7+1.8.9-forge.jar`.
3. On the Lunar home screen, press **`O`** to open the account switcher (you should briefly see `IAS loaded - press O` in the top-left if the mod loaded).
